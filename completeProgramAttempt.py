...
# missing in the first quadrant, which I then plug in to the zeros
...
# +This i for loop gets me the solutions to the sudoku puzzle
# in the first quadrant (top left)
...
# take those and run them in function below someway. Reduce overall
                ...
                for f in range(0, 9, 1):
                    for t in range(0, 9, 1):
                        ...
                        if (q.checkVerticalFinal(t, guess[f, t]) != 1 or
                            ...
                            if counter2 >= 81:
                                answer1.append(unitedList1[i])
                                answer2.append(unitedList2[i])
                                answer3.append(unitedList3[i])
                                answer4.append(unitedList4[j])
                                answer5.append(unitedList5[j])
                                answer6.append(unitedList6[j])
...
for u in range(6,9,1):
    for w in range(0,3,1):
        if finalSolution[u,w] == 0:
            finalSolution[u,w] = quad7List[answer7[0]][count7]
            count7 += 1
    for e in range(3,6,1):
        if finalSolution[u,e] == 0:
            finalSolution[u,e] = quad8List[answer8[0]][count8]
            count8 += 1
    for r in range(6,9,1):
        if finalSolution[u,r] == 0:
            finalSolution[u,r] = quad9List[answer9[0]][count9]
            count9 += 1
